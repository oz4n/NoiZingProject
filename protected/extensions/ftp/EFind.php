...
// +----------------------------------------------------------------------+
//
// $Id: EFind.PHP 304090 2010-10-05 14:07:34Z clockwerx $
//

require_once 'PEAR.php';

define('EFind_VERSION', '@package_version@');
 ...
 *
 * @access public
 * @version $Id: EFind.PHP 304090 2010-10-05 14:07:34Z clockwerx $
 * @package File
 * @author Sterling Hughes <sterling@php.net>
 */
class EFind extends CApplicationComponent {
    ...
    function glob($pattern, $dirpath, $pattern_type = 'php') {
        ...
        $match_function = EFind::_determineRegex($pattern, $pattern_type);
        $matches = array();
        ...
        while (list($key, $val) = each($retval)) {
            ...
            if (!is_array($val) && is_dir($path)) {
                ...
                if ($maxrecursion == 0 || $count < $maxrecursion) {
                    $retval[$val] = EFind::mapTreeMultiple($path, $maxrecursion, $count);
                }
            }
        }
    ...
    function search($pattern, $directory, $type = 'php', $fullpath = true, $match = 'files') {
        ...
        $matches = array();
        list ($directories, $files) = EFind::maptree($directory);
        switch ($match) {
            case 'directories':
                $data = $directories;
                break;
        ...
        $match_function = EFind::_determineRegex($pattern, $type);
     ...
     * use, implemented by EFind::glob() and EFind::search()
     *
    ...
    function _determineRegex($pattern, $type) {
        if (!strcasecmp($type, 'shell')) {
            $match_function = 'EFind_match_shell';
 ...
 */
function EFind_match_shell($pattern, $filename) {
    ...
    $positive = _EFind_match_shell_get_pattern($positive);
    if ($negation) {
        $negative = _EFind_match_shell_get_pattern($negative);
    }
    // }}} convert end 


    if (defined("EFind_DEBUG")) {
        print("Method: $type\nPattern: $pattern\n Converted pattern:");
        print_r($positive);
        if (isset($negative))
            print_r($negative);
...
/**
 * function used by EFind_match_shell to convert 'shell' mask
 ...
 */
function _EFind_match_shell_get_pattern($mask) {
