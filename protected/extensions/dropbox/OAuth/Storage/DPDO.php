...
class DPDO extends Session {
    ...
    public function __construct(Encrypter $encrypter = null, $userID) {
        ...
        if (!extension_loaded('PDO')) {
            throw new Exception('This storage handler requires the PDO extension');
        }
     ...
     * @return void
     */
    public function dbConnect($host, $db, $user, $pass, $port = 3306) {
        $dsn = 'mysql:host=' . $host . ';port=' . $port . ';dbname=' . $db;
        $this->pdo = new PDO($dsn, $user, $pass, $this->options);
    }
    
    /**
     * Yii database Connect to the database
     * @param string $dsn database conection string
     * @param string $user Database username
     * @param string $pass Database user password
     * @param int $port Database server port (Default: 3306)
     * @return void
     */
    public function yiiDbConnect($dsn, $user, $pass) {        
        $this->pdo = new PDO($dsn, $user, $pass, $this->options);
    }
