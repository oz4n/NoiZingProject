...
class TbModal extends CWidget
{
	...
	public function run()
	{
		...
		$cs->registerScript(__CLASS__.'#'.$id, "jQuery('#{$id}').modal({$options}).css({'width': function () { return ($(document).width() * .".$this->width.") + 'px'; },'margin-left': function () { return -($(this).width() / 2); }});");
