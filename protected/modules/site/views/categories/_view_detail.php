...
<div class="blog margin-bottom-30">
	...
	<?php if ($categories->post_status === 'info'): ?>
		<div style="margin-top:-20px;">
			<ul class="unstyled inline blog-tags">
				<li>
					<i class="icon-calendar"></i><a><?php echo date('F j, Y', $categories->create_time); ?></a>
				</li>
				<li>
					<i class="icon-user"></i> <?php echo CHtml::link($categories->account->username, 'mailto:' . $categories->account->email); ?>
				</li>
				<li>
					<i class="icon-comment"></i> <a>With (<?php echo $categories->commentCount; ?>) Comments </a>
				</li>
			</ul>
		</div>
	<?php endif; ?>
	...
	<?php if ($categories->post_status === 'info'): ?>
		<div style="margin-top: 20px; padding-top: 3px; border-top: 1px solid #eee">
			<ul class="unstyled inline blog-tags">
				<li><i class="icon-th-list"></i> <?php echo implode(", ", $categories->catlinks($categories->id)); ?>
				</li>
				<li><i class="icon-tags"></i> <?php echo implode(', ', $categories->taglinks); ?></li>
			</ul>
		</div>
	<?php endif; ?>
</div>
<?php if ($categories->comment_status == "E"): ?>
	<div class="comment-chat">
		<b><i class="icon-comment"></i> Comments (0)</b>
		<ul class="chat-box timeline">
			<?php foreach ($comment->findAllByPostID($categories->id) as $com): ?>
				<li class="arrow-box-left gray">
					...
					<div class="content">
						...
						<?php foreach ($comment->findAllByPostID($categories->id, $com->id) as $comc): ?>
