...
<div class="blog margin-bottom-30">
	...
	<?php if ($pages->post_status === 'info'): ?>
		<div style="margin-top:-20px;">
			<ul class="unstyled inline blog-tags">
				<li><i class="icon-calendar"></i><a><?php echo date('F j, Y', $pages->create_time); ?></a></li>
				<li>
					<i class="icon-user"></i> <?php echo CHtml::link($pages->account->username, 'mailto:' . $pages->account->email); ?>
				</li>
				<li><i class="icon-comment"></i> <a>With (<?php echo $pages->commentCount; ?>) Comments </a></li>
			</ul>
		</div>
	<?php endif; ?>
	...
	<?php if ($pages->post_status === 'info'): ?>
		<div style="margin-top: 20px; padding-top: 3px; border-top: 1px solid #eee">
			<ul class="unstyled inline blog-tags">
				<li><i class="icon-th-list"></i> <?php echo implode(", ", $pages->catlinks($pages->id)); ?></li>
				<li><i class="icon-tags"></i> <?php echo implode(', ', $pages->taglinks); ?></li>
			</ul>
		</div>
	<?php endif; ?>
</div>
<?php if ($pages->comment_status == "E"): ?>
	<div class="comment-chat">
		<b><i class="icon-comment"></i> Comments (0)</b>
		<ul class="chat-box timeline">
			<?php foreach ($comment->findAllByPostID($pages->id) as $com): ?>
...
<!--						<img class="avatar-small" src="http://beer2code.com/themes/core-admin/images/avatars/avatar1.jpg">-->
					...
					<div class="content">
						...
						<?php foreach ($comment->findAllByPostID($pages->id, $com->id) as $comc): ?>
