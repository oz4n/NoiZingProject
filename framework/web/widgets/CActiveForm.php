 ...
 * <ul>
 * <li>server-site validation: the validation is performed at server site after
 ...
 * an AJAX request is triggered which requires server-site validation. The validation
 * result is sent back in AJAX response and the input field changes its appearance
 * accordingly.</li>
 * <li>client-site validation (available since version 1.1.7):
 * when the user enters data into an input field,
 * validation is performed on the client site using JavaScript. No server contact
 ...
 * message content.
 *
 * To ensure data validity, server-site validation is always performed.
 * By setting {@link enableAjaxValidation} to true, one can enable AJAX-based validation;
 * and by setting {@link enableClientValidation} to true, one can enable client-site validation.
 * Note that in order to make the latter two validations work, the user's browser
 * must has its JavaScript enabled. If not, only the server-site validation will
 * be performed.
 *
 * The AJAX-based validation and client-site validation may be used together
 * or separately. For example, in a user registration form, one may use AJAX-based
 * validation to check if the user has picked a unique username, and use client-site
 ...
 * if possible, one should mainly use client-site validation.
 *
 ...
 * may cause server-site state changes. Third, it is not designed
 * to work with tabular data input for the moment.
 *
 * Support for client-site validation varies for different validators. A validator
 * will support client-site validation only if it implements {@link CValidator::clientValidateAttribute}
 * and has its {@link CValidator::enableClientValidation} property set true.
 * At this moment, the following core validators support client-site validation:
 * <ul>
...
class CActiveForm extends CWidget
{
	 ...
	 * The server site can then detect who triggers the AJAX validation and react accordingly.
	 * Defaults to 'ajax'.</li>
	 ...
	 * the jquery representation of the form object; 'data' is the JSON response from the server-site validation; 'hasError'
	 ...
	 * 'data' is the JSON response from the server-site validation; 'hasError' is a boolean value indicating whether
	 ...
	 * When this property is set true, you should respond to the AJAX validation request on the server site as shown below:
	 * <pre>
	 * public function actionCreate()
	 * {
	 *     $model=new User;
	...
	/**
	 * @var boolean whether to enable client-site data validation. Defaults to false.
	 *
	 * When this property is set true, client-site validation will be performed by validators
	 ...
	 * is set true.
	 *
	 * When client-site validation is enabled, an option named "clientValidation" is also recognized.
	 * This option should take a piece of JavaScript code to perform client-site validation. In the code,
	 ...
	 * <li>attribute: a data structure keeping all client-site options for the attribute</li>
	 * </ul>
	 * @param boolean $enableAjaxValidation whether to enable AJAX validation for the specified attribute.
	 * Note that in order to enable AJAX validation, both {@link enableAjaxValidation} and this parameter
	 * must be true.
	 * @param boolean $enableClientValidation whether to enable client-site validation for the specified attribute.
	 * Note that in order to enable client-site validation, both {@link enableClientValidation} and this parameter
